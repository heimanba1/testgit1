 ...
 <div class="container" >
       ...
       <table class="table table-condensed table-bordered table-striped" id="tableid">
	      ...
	      <tbody id="tbody">
	        <s:iterator var="allindustry" value="listIndustry">
	          ...
	          <table>
    <form action="${pageContext.request.contextPath }/industryAction!queryIndustryByGroupname.go?i_groupname=${allindustry.i_groupname}" method="post" style="float: left;">
	    <tr>
    ...
    <div class="STYLE9"">
      <td  width="12" align="center">页次:${currentPage}/${sumPage}页&nbsp;每页${pageSize}条信息共${count}条
        ...
        <a class="STYLE9" href="${pageContext.request.contextPath}/industryAction!getIndustryByGroupname_all1.go?currentPage=1&&i_groupname=${allindustry.i_groupname}">首页</a>
		
		<s:if test="currentPage==1">上一页</s:if>
		
		
		<s:else>
		<a class="STYLE9" href="${pageContext.request.contextPath }/industryAction!getIndustryByGroupname_all1.go?currentPage=${currentPage-1}&&i_groupname=${allindustry.i_groupname}">上一页</a>
		</s:else>
		
		<s:if test="currentPage>=sumPage">下一页</s:if>
		
		<s:else>
		<a class="STYLE9" href="${pageContext.request.contextPath }/industryAction!getIndustryByGroupname_all1.go?currentPage=${currentPage+1}&&i_groupname=${allindustry.i_groupname}">下一页</a>
		</s:else>
		
		
		<a class="STYLE9" href="${pageContext.request.contextPath }/industryAction!getIndustryByGroupname_all1.go?currentPage=${sumPage}&&i_groupname=${allindustry.i_groupname}">尾页</a></td>
    </div>
  </div>
